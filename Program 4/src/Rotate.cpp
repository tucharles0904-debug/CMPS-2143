...
ID: 10-P03
Description: Implements clockwise rotation by 0, 90, 180, or 270 degrees.
             Some rotations require building a new grid with swapped dimensions.
*/

Rotate::Rotate(int angle) {
    this->angle = angle;
}
...
void Rotate::apply(Grid& pixels) {
    ...
    if (angle == 180) {
        Grid result(height, std::vector<Color>(width));
        ...
        Grid result(width, std::vector<Color>(height));
